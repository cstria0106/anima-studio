CREATE TABLE `__repaired_model_downloads` (
...
INSERT INTO `__repaired_model_downloads` (
  `id`,
  `operation_id`,
  `state`,
  `provider`,
...
SELECT
  ...
  `provider_download_id`,
  CASE
    WHEN trim(`provider_model_id`) <> '' THEN `provider_model_id`
    ELSE CAST(`model_id` AS text)
  END,
  CASE
    WHEN trim(`provider_version_id`) <> '' THEN `provider_version_id`
    ELSE CAST(`model_version_id` AS text)
  END,
  CASE
    WHEN `provider_file_id` IS NOT NULL AND trim(`provider_file_id`) <> ''
      THEN `provider_file_id`
...
--> statement-breakpoint
ALTER TABLE `__repaired_model_downloads` RENAME TO `model_downloads`;
--> statement-breakpoint
